...
const Details = () => {
  ...
  const [dataMovie] = useRequestData(
    `/movie/${params.id}?api_key=${api_Key}&language=pt-BR`
  );
  const [dataCrew] = useRequestData(
    `/movie/${params.id}/credits?api_key=${api_Key}&language=pt-BR`
  );
  const [trailer] = useRequestData(
    `/movie/${params.id}/videos?api_key=${api_Key}&language=pt-BR`
  );
  const [recommended] = useRequestData(
    `/movie/${params.id}/recommendations?api_key=${api_Key}&language=pt-BR&page=1`
  );
  const navigate = useNavigate();

  const crewList =
    dataCrew &&
    dataCrew.cast.map((person) => {
      let photo = "";
      ...
      for (let i = 0; i < 6; i++) {
        ...
        array.push(
          <Movie key={recommended[i].id}>
            <MovieImg
              src={`${urlBaseImgs}/w500/${recommended[i].poster_path}`}
            />
            <Ptitle
              onClick={() => goToMovieDetail(navigate, recommended[i].id)}
            >
